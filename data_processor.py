...
class AIModelHandler:
    ...
    def generate_qa_pair(self, context: str, related_contexts: List[str] = None) -> Optional[Dict[str, Any]]:
        """使用AI生成多跳问答对"""
        if not self.agent:
            return None
...
class UserDataProcessor:
    ...
    def __init__(self, config: ProcessorConfig = None):
        ...
        self.ai_handler = AIModelHandler(self.config) if self.config.use_ai_model else None
        
    def process_text(self, text: str, source: str = "user_input") -> List[Dict[str, Any]]:
        """处理单个文本"""
        # 将文本转换为标准格式
        raw_data = [{
            'text': text,
    ...
    def process_batch(self, texts: List[str], sources: List[str] = None) -> List[Dict[str, Any]]:
        """批量处理多个文本"""
        if sources is None:
...
class ExampleConstructor:
    """示例构造器"""
    def __init__(self, config: ProcessorConfig, ai_handler: Optional[AIModelHandler] = None):
        self.config = config
        self.ai_handler = ai_handler
    
    def construct_examples(self, raw_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """构造训练示例"""
    ...
    def _generate_qa_pairs(self, info_pairs: List[Dict[str, str]]) -> List[Dict[str, str]]:
        """生成多跳问答对"""
        qa_pairs = []
...
class DataCurator:
    ...
    def curate_dataset(self, examples: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """数据集管理"""
        logger.info("开始数据集管理...")
    ...
    def _quality_filter(self, examples: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """质量过滤"""
        filtered = []
    ...
    def _check_qa_quality(self, qa_pairs: List[Dict[str, str]]) -> bool:
        ...
        for qa in qa_pairs:
            # 1. 长度检查
            if len(qa.get('question', '')) < 10 or len(qa.get('answer', '')) < 5:
                return False
    ...
    def _complexity_filter(self, examples: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """复杂度过滤"""
        return [
            example for example in examples
            if example.get('metadata', {}).get('complexity', 0) >= self.config.complexity_threshold
        ]
    
    def _remove_duplicates(self, examples: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """去重"""
        seen = set()
        unique_examples = []
    ...
    def _sample_dataset(self, examples: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """采样到目标数据集大小"""
